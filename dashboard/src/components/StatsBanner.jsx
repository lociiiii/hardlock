  ...
  const items = [
    {
      ...
      icon: KeyIcon,
      color: 'text-hardlock-accent',
    },
    {
      label: 'Active Devices',
      value: stats?.active_devices ?? '—',
      icon: DeviceIcon,
      color: 'text-hardlock-400',
    },
    {
      label: 'Launches Today',
      value: stats?.launches_today ?? '—',
      icon: RocketIcon,
      color: 'text-hardlock-success',
    },
    {
      label: 'Launches This Week',
      value: stats?.launches_this_week ?? '—',
      icon: ChartIcon,
      color: 'text-hardlock-warning',
    },
  ];
    ...
    <div className="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
      {items.map(({ label, value, icon: Icon, color }) => (
        <div key={label} className="card flex items-center gap-4 !p-4">
          <div className={`flex h-10 w-10 items-center justify-center rounded-lg bg-hardlock-800 ${color}`}>
            <Icon />
          </div>
          <div>
            ...
            <p className="text-2xl font-bold text-white">
              {loading ? (
                <span className="inline-block h-7 w-12 animate-pulse rounded bg-hardlock-700" />
              ) : (
                value
              )}
            </p>
